...
parser.add_argument("--start_CMI_epoch", type=int, default=1)
...
args.log_name += 'FADES_{:.1f}_{:.1f}_{:.1f}_{:.1f}_{:.1f}_{:.1f}_{:d}'.format(args.bs, args.lambda_cls, args.lambda_tc,
                                                                               args.lambda_fair,
...
save_dir = os.path.join('./save_direct', dataname, args.log_name)
...
testloader = DataLoader(test_data, batch_size=bs_val, shuffle=False, num_workers=32, pin_memory=True)
...
joint_cls = nn.DataParallel(JointClassifier(input_dim=3 * feat_dim)).cuda()
...
if args.resume:
    ...
    joint_cls.load_state_dict(torch.load(os.path.join(save_dir, 'joint_cls.pth'))['state_dict'])
...
optimizer_joint = torch.optim.Adam(joint_cls.module.get_parameters(), lr=1e-3, weight_decay=1e-4)
...
for epoch in range(start_epoch, epochs + 1):
    if epoch > 15 and epoch <= 50:
        ...
        optimizer_joint = torch.optim.Adam(joint_cls.module.get_parameters(), lr=5e-4, weight_decay=1e-4)
        ...
        optimizer_joint = torch.optim.Adam(joint_cls.module.get_parameters(), lr=1e-4, weight_decay=1e-4)
        optimizer_d = torch.optim.Adam(Dis.parameters(), lr=1e-5, weight_decay=1e-5)
    ...
    trainloader_2 = iter(
        DataLoader(train_data, batch_size=bs, shuffle=True, drop_last=True, num_workers=32, pin_memory=True))
    ...
    for x_batch, s_batch, y_batch in iters:
        ...
        joint_label = (y_batch.view(-1) * 2 + s_batch.view(-1)).long()  # map (y,s) to {0,1,2,3}
        logits_joint = joint_cls(torch.cat([z_y, z_s, z_r], dim=-1))
        joint_loss = criterion_ce(logits_joint, joint_label)

        # separate classification loss
        ...
        cls_loss = args.alpha * (loss_y + loss_a) + joint_loss

        # mutual information loss
        loss_CMI = Joint_CMI_loss(joint_cls, cls_y, cls_a, z_y, z_s, z_r, args.beta)
        ...
        if epoch > args.start_CMI_epoch: # epoch > start_CMI_epoch, introduce CMI loss
            ...
            for params in cls_a.module.parameters():
                params.require_grad = False
            for params in joint_cls.module.parameters():
                params.require_grad = False
        ...
        iters.set_description(
            'epoch : {}, Recon : {:.3f}, Prior : {:.3f}, CMI loss : {:.3f}, TC loss : {:.3f}, Y loss : {:.3f}, A loss : {:.3f}' \
                .format(epoch, loss_recon_hist / step, loss_prior_hist / step, loss_fair_hist / step,
                        loss_tc_hist / step,
                        loss_y_hist / step, loss_a_hist / step))

    write_loss_to_log(
        '\nepoch : {}, Recon : {:.3f}, Prior : {:.3f}, CMI loss : {:.3f}, TC loss : {:.3f}, Y loss : {:.3f}, A loss : {:.3f}' \
            .format(epoch, loss_recon_hist / step, loss_prior_hist / step, loss_fair_hist / step, loss_tc_hist / step,
                    loss_y_hist / step, loss_a_hist / step))
    ...
    if epoch % 10 == 0:
        ...
        torch.save({'state_dict': cls_y.state_dict(), 'epoch': epoch}, os.path.join(save_dir, f'cls_y_{epoch}.pth'))
        torch.save({'state_dict': cls_a.state_dict(), 'epoch': epoch}, os.path.join(save_dir, f'cls_a_{epoch}.pth'))
        torch.save({'state_dict': joint_cls.state_dict(), 'epoch': epoch},
                   os.path.join(save_dir, f'joint_cls_{epoch}.pth'))
    ...
    torch.save({'state_dict': joint_cls.state_dict(), 'epoch': epoch}, os.path.join(save_dir, 'joint_cls.pth'))
